...
export default async function BudgetDetailsPage({params,}: {params: { id: string }}) {
    ...
    return (
        ...
        {budget.expenses.length ? (
            <>
            <h1 className="font-black text-4xl text-purple-900 mt-10">Gastos</h1>
            <ul
                role="list"
                className="divide-y divide-gray-300 border border-gray-300 rounded-lg shadow-lg mt-10 "
            >
                ...
                <li key={expense.id} className="flex justify-between gap-x-6 p-5">
                    <div className="flex min-w-0 gap-x-4">
                        <div className="min-w-0 flex-auto space-y-2">
                            ...
                            <p className="text-gray-500  text-sm">Última actualización: <span className="font-bold">{formatDate(expense.updatedAt)}</span></p>
                        </div>                        
                    </div>
                    <ExpenseMenu/>
                </li>
