...
async function getUserBudgets() {
  ...
  const budgets = BudgetsAPIResponseShcema.parse(json);
    ...
    <>
      <div className="flex flex-col-reverse md:flex-row md:justify-between items-center">
        ...
        <Link
          ...
          className="bg-amber-500 p-2 text-white font-bold w-full md:w-auto text-center rounded-xl"
        >
          Crear Presupuesto
        </Link>
      </div>
      ...
      {budgets.length ? (
        <>
          <ul
            role="list"
            className="divide-y divide-gray-300 border border-gray-300 shadow-lg mt-10 rounded-xl"
          >
            {budgets.map((budget) => (
